#!/usr/bin/env node
// Decides which independent developer-tool packages should be published on
// this push, and at which npm dist-tag, under the channel-promotion scheme:
...
//
// Packages are emitted in dependency order so a max-parallel:1 matrix publishes
// @mentra/miniapp-cli before the wrappers that pin it.
...
const PACKAGES = [
  ...
  },
  {
    ...
    alsoRequires: ["@mentra/miniapp-cli"],
  },
  {
...
// First pass: read every independent tool manifest and its registry state once.
const family = {}
...
console.log(
  `Publishing ${matrix.length}/${PACKAGES.length} developer tool(s). Dry run: ${dryRun}. Force: ${forceRelease}.`,
)
