...
export function MicPcmLogger() {
  ...
  useBluetoothEvent('mic_health', (event) => {
    ...
    console.log(event.lastLc3ReceivedAt, event.lastPcmProducedAt);
  });
...
Native Android and iOS apps receive `MicPcmEvent` and `MicLc3Event` objects with the same metadata through the SDK listener/delegate surfaces. They also receive typed `MicHealthEvent` events through `onMicHealth` on Android and `.micHealth` on iOS.

`mic_health` fires only when native processing detects an LC3 `sequence_gap` or `decode_failure`. It includes cumulative `sequenceGapEvents` and `decodeFailures` counters for the current connection, plus `lastLc3ReceivedAt`, `lastPcmProducedAt`, and the event `timestamp` in Unix milliseconds when available. Use the timestamps with your own stall policy; the SDK does not emit a separate stall event. Disable microphone audio callbacks when the app finishes using them.
