...
If your app uses SDK `{{release-version}}`, [install the matching Mentra Live `{{release-version}}` software](/bluetooth-sdk/software-update), then reconnect and check again. Do not debug individual feature failures until the OTA check succeeds with no update remaining.
